...
void main() {
  group(
    'OpenAI Responses Session Persistence Tests',
    () {
      ...
      test(
        ...
        () async {
          ...
          expect(
            ...
            isFalse,
            reason:
                'Standard OpenAI should not have Responses session metadata',
          );

          // Step 3: Use another different provider (if available) We'll use
          // OpenAI again but it could be any non-Responses provider
          ...
          // Step 4: Return to OpenAI Responses (should find session from step
          // 1)
          ...
          expect(
            ...
            isTrue,
            reason:
                'Should have tracked message count for second Responses call',
          );

          // Should send only messages after the first Responses call History
          // has: user(1), model(1), user(2), model(2), user(3), model(3),
          // user(4) Should send from index 2 onwards = 5 messages
          expect(
            sentMessageCounts.first,
            equals(5),
            reason:
          ...
          // Step 5: Third OpenAI Responses call (should find session from step
          // 4)
          sentMessageCounts.clear();
          previousResponseIds.clear();
          ...
          expect(
            ...
            isTrue,
            reason:
                'Should have tracked message count for third Responses call',
          );

          // Should send only messages after the second Responses call Only the
          // new user message since last Responses call
          expect(
            sentMessageCounts.first,
            equals(1),
      ...
      test(
        ...
        () async {
          ...
          expect(
            ...
            equals(secondSessionId),
            reason:
                'Should use most recent Responses session, not the first one',
          );
